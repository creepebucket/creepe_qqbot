...
from redstone_daily.plugins.utils import get_context, permission_required, check_command_enabled, get_database
...
# 命令元信息（用于 /help 聚合）
add_info('set_role_nyat', 'NYAT: 设置角色，参数: /set_role_nyat <角色名> <角色设定>')
add_info('role_nyat', 'NYAT: 查看当前角色')
add_info('calm_nyat', 'NYAT: 冷静，清空全局情绪')
add_info('reset_nyat', 'NYAT: 重置会话')
add_info('status_nyat', 'NYAT: 查看状态信息')
add_info('presets_nyat', 'NYAT: 查看可用预设')
add_info('set_preset_nyat', 'NYAT: 设置预设，参数: /set_preset_nyat <文件名>')
add_info('list_groups_nyat', 'NYAT: 查看启用nyabot的群组列表')

help = on_command(rule=is_group_message, cmd="help_nyat", aliases={"帮助"}, priority=0, block=True)
help_pm = on_command(
    rule=is_private_message, cmd="help_nyat", aliases={"帮助"}, priority=0, block=True
)
get_status = on_command(
    rule=is_group_message, cmd="status_nyat", aliases={"状态"}, priority=0, block=True
)
get_status_pm = on_command(
    rule=is_private_message, cmd="status_nyat", aliases={"状态"}, priority=0, block=True
)
auto_chat = on_message(rule=is_group_message, priority=1, block=False)
set_role = on_command(
    rule=is_group_message, cmd="set_role_nyat", aliases={"设置角色"}, priority=0, block=True
)
set_role_pm = on_command(
    rule=is_private_message, cmd="set_role_nyat", aliases={"设置角色"}, priority=0, block=True
)
get_role = on_command(
    rule=is_group_message, cmd="role_nyat", aliases={"当前角色"}, priority=0, block=True
)
get_role_pm = on_command(
    rule=is_private_message, cmd="role_nyat", aliases={"当前角色"}, priority=0, block=True
)
calm_down = on_command(
    rule=is_group_message, cmd="calm_nyat", aliases={"冷静"}, priority=0, block=True
)
calm_down_pm = on_command(
    rule=is_private_message, cmd="calm_nyat", aliases={"冷静"}, priority=0, block=True
)
reset = on_command(rule=is_group_message, cmd="reset_nyat", aliases={"重置"}, priority=0, block=True)
reset_pm = on_command(
    rule=is_private_message, cmd="reset_nyat", aliases={"重置"}, priority=0, block=True
)
get_presets = on_command(
    rule=is_group_message, cmd="presets_nyat", aliases={"preset"}, priority=0, block=True
)
get_presets_pm = on_command(
    rule=is_private_message, cmd="presets_nyat", aliases={"preset"}, priority=0, block=True
)
set_presets = on_command(
    rule=is_group_message, cmd="set_preset_nyat", aliases={"set_presets"}, priority=0, block=True
)
set_presets_pm = on_command(
    rule=is_private_message, cmd="set_preset_nyat", aliases={"set_presets"}, priority=0, block=True
)
list_groups_pm = on_command(
    rule=is_private_message, cmd="list_groups_nyat", aliases={"群组列表"}, priority=0, block=True
)
...
async def handle_get_presets(event: GroupMessageEvent):
    ...
    await do_get_presets(get_presets, group.id)
...
async def handle_set_presets(event: GroupMessageEvent):
    user, args, group = get_context(event)
    if not args:
        await set_presets.finish("请提供<预设文件名>")
    file = args[0]
    await do_set_presets(set_presets, group.id, file)
...
@set_role.handle()
@permission_required(3)
async def handle_set_role(event: GroupMessageEvent):
    user, args, group = get_context(event)
    if len(args) < 2:
        await set_role.finish("请提供<角色名> <角色设定>")
    name = args[0]
    role = " ".join(args[1:])
    await do_set_role(set_role, group.id, name, role)
...
async def handle_get_role(event: GroupMessageEvent):
    user, args, group = get_context(event)
    await do_get_role(get_role, group.id)
...
async def handle_calm_down(event: GroupMessageEvent):
    user, args, group = get_context(event)
    await do_calm_down(calm_down, group.id)
...
async def handle_status(event: GroupMessageEvent):
    user, args, group = get_context(event)
    await do_status(get_status, group.id)
...
async def handle_list_groups_pm():
    # 按指令开关系统列出已启用 nyaturingtest_chat 的群
    group_commands = get_database('group_commands').collection
    cursor = group_commands.find({'allowed_commands': 'nyaturingtest_chat'})
    groups = [str(doc.get('group_id')) for doc in cursor]
    if not groups:
