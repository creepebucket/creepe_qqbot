...
class Session:
    ...
    def __init__(self, siliconflow_api_key: str, id: str = "global", name: str = "terminus"):
        ...
        """
        # MongoDB 集合
        self._db = get_database("nyaturingtest_sessions")
    ...
    def save_session(self):
        ...
        try:
            ...
            # 写入MongoDB（upsert）
            self._db.update_one({"id": self.id}, {"$set": session_data}, upsert=True)

            logger.debug(f"[Session {self.id}] 会话状态已保存到MongoDB")
        except Exception as e:
            logger.debug(f"[Session {self.id}] 保存会话状态到MongoDB失败: {e}")
    ...
    def load_session(self):
        ...
        """
        try:
            session_data = self._db.get({"id": self.id})
            if not session_data:
                logger.debug(f"[Session {self.id}] 会话记录不存在，使用默认状态")
            return
            ...
            logger.info(f"[Session {self.id}] 会话状态已从MongoDB加载")
        except Exception as e:
