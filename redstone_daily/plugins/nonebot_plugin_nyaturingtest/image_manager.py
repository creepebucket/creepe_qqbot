...
class ImageManager:
    ...
    def __init__(self):
        if not self._initialized:
            ...
            # MongoDB 集合（图片缓存）
            self._db = get_database("nyaturingtest_image_cache")
    ...
    async def get_image_description(self, image_base64: str, is_sticker: bool) -> ImageWithDescription | None:
        ...
        # 先查 MongoDB 缓存
        try:
            cached = self._db.get({"_id": image_hash})
            if cached:
                try:
                    image_with_desc = ImageWithDescription.from_json(cached.get("data", ""))
                    if image_with_desc.is_sticker != is_sticker:
                        image_with_desc.is_sticker = is_sticker
                        # 更新缓存
                        self._db.update_one(
                            {"_id": image_hash},
                            {"$set": {"data": image_with_desc.to_json(), "updated_at": _now_str()}},
                            upsert=True,
                        )
                    return image_with_desc
                except ValueError as e:
                    logger.error(f"Mongo缓存数据格式错误，将重新生成: {e}")
                    # 删除坏数据
                    self._db.delete_one({"_id": image_hash})
        except Exception as e:
            logger.error(f"读取Mongo缓存失败: {e}")
        ...
        try:
            self._db.update_one(
                {"_id": image_hash},
                {"$set": {"data": result.to_json(), "updated_at": _now_str()}},
                upsert=True,
            )
        except Exception as e:
            logger.error(f"写入Mongo缓存失败: {e}")
